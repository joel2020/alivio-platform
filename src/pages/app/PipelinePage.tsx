...
import { fetchMatchedCandidates, updateCandidateMatchStatus, type CandidateRoleMatchStatus, type MatchedCandidate } from '../../lib/candidateMatches';
import Toast from '../../components/app/Toast';

type PipelineCandidate = Candidate | MatchedCandidate;
...
function hasRoleMatch(candidate: PipelineCandidate): candidate is MatchedCandidate {
  return 'match_id' in candidate;
}

function getDisplayScore(candidate: PipelineCandidate) {
  if (hasRoleMatch(candidate)) return Math.round(candidate.match_score * 100);
  return candidate.ai_score ?? candidate.score;
}
...
export default function PipelinePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      try {
        candidateData = await fetchMatchedCandidates(id);
      } catch (matchError) {
        console.warn('Falling back to role candidates after candidate match load failed', matchError);
        const candRes = await supabase.from('candidates').select('*').eq('role_id', id).order('score', { ascending: false });
        if (candRes.error) {
          const authError = /jwt|permission|not authenticated|forbidden|auth/i.test(candRes.error.message);
          if (authError) {
            setUnauthorized(true);
          } else {
            setLoadError(candRes.error.message);
          }
          return;
        }
        candidateData = candRes.data || [];
      }
  ...
  const filteredCandidates = candidates
    ...
    .sort((a, b) => {
      const aVal = sortField === 'score' ? getDisplayScore(a) ?? 0 : a[sortField] ?? 0;
      const bVal = sortField === 'score' ? getDisplayScore(b) ?? 0 : b[sortField] ?? 0;
  ...
  async function handleMatchStatusChange(matchId: string, status: CandidateRoleMatchStatus) {
    try {
      await updateCandidateMatchStatus(matchId, status);
      setCandidates((prev) => prev.map((candidate) => (
        hasRoleMatch(candidate) && candidate.match_id === matchId
          ? { ...candidate, match_status: status, submitted_at: status === 'submitted' ? new Date().toISOString() : candidate.submitted_at }
          : candidate
      )));
      setToast('Candidate match status updated.');
    } catch (error) {
      setToast(`Could not update match status: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
                            ...
                            {matched && (
                              <select
                                ...
                                onChange={(event) => {
                                  ...
                                  void handleMatchStatusChange(c.match_id, event.target.value as CandidateRoleMatchStatus);
                                }}
                                style={{
