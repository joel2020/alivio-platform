...
export default function ContactPage() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await supabase.functions.invoke('send-outreach-email', { body: { to: 'hello@aliviosearchpartners.com', subject: `Website inquiry from ${name || 'Healthcare hiring team'}`, body: `Name: ${name}\nEmail: ${email}\nOrganization: ${facility}\n\nMessage:\n${message}`, from: 'Alivio Contact Form <noreply@aliviosearchpartners.com>' } });
        ...
        <div className="grid gap-8 md:grid-cols-2">
          <form onSubmit={onSubmit} className="rounded-xl border p-6" style={{ borderColor: '#1E1E1E', backgroundColor: '#141414' }}>
            ...
            <div className="space-y-4">
              ...
              <button type="submit" disabled={isSubmitting} className="inline-block px-6 py-3 rounded font-medium text-white" style={{ backgroundColor: '#4F46E5' }}>{isSubmitting ? 'Sending...' : 'Send Message'}</button>
            </div>
          </form>
