...
import { practices, searchSteps, internationalSearches } from '../../lib/recruitingContent';
...
export function InternationalSearches() {
  return <div id="international-recruiting" className="recruiting-international">
    <p className="firm-eyebrow">LATAM & offshore recruitment</p>
    <h2>Find the right people beyond borders.</h2>
    <div className="firm-values">{internationalSearches.map(service => <article key={service.href}>
      <h3><Link to={service.href}>{service.title}</Link></h3><p>{service.copy}</p>
      <Link to={service.href} className="firm-text-link">Explore {service.title} <ArrowUpRight size={18} aria-hidden="true" /></Link>
    </article>)}</div>
  </div>;
}
