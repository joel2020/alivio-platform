...
Deno.serve(async (req: Request) => {
  ...
  const auth = await requireFunctionAuth(req, "ai-parse-email-resume");
  ...
  try {
    ...
    if (resume_attachment_id) {
      const { data: attachment, error: attachmentError } = await supabase
        ...
        .select("id, email_id, file_content, candidate_id")
      ...
      if (attachment.candidate_id) {
        return new Response(JSON.stringify({ candidate_id: attachment.candidate_id, skipped: true, reason: "already_parsed" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }

      try {
        textContent = atob(attachment.file_content || "");
      } catch {
        throw new Error("Malformed attachment encoding");
      }
    ...
    const { data: inboxEmail, error: emailError } = await supabase
      .from("email_inbox")
      .select("id, org_id, subject, body_text, candidate_id")
      .eq("id", emailId)
      .single();
    if (emailError || !inboxEmail) throw emailError ?? new Error("Email not found");

    if (inboxEmail.candidate_id) {
      return new Response(JSON.stringify({ candidate_id: inboxEmail.candidate_id, skipped: true, reason: "email_already_processed" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    const ai = await callAiWithFallback({ prompt: textContent.slice(0, 12000), systemPrompt, temperature: 0, timeoutMs: 75_000 });
    const parsed = JSON.parse(ai.content || "{}") as ParsedCandidate;
