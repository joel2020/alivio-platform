...
async function searchVertex(body, requestId) {
  const { query, pageSize } = parseSearchInput(body, config.vertex);

  if (isMockMode) {
    return buildMockResponse(mockVertexFixture, query, requestId);
  }
...
async function searchRecruiter(body, requestId) {
  if (isMockMode) {
...
function normalizeOpenAiError(error, requestId) {
  if (error?.status >= 400) {
    return error;
  }
